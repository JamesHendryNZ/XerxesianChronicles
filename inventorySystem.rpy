    ...
    if len(useableItems) > 0:
        ...
        if usedItem.item.itemType == "grapple" or usedItem.item.itemType == "grappleNet":
            ...
            if usedItem.item.effectDuration > 0:
                ...
                if usedItem.item.effectType == "Clears":
                    play sound clearMyMind
                    "[dude2Heal.name] is cleared of all negative effects and will protect them for [usedItem.item.effectDuration] turns."
                    $ removeBadEffects( dude2Heal.effects , dude2Heal )
