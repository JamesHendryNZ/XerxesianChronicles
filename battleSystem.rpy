init python:
    ...
    def getFoeTypeByName( enemyTroopers , nameOfFoe , howMany = 1 ):

        extraFoes = []
        for foe in enemyTroopers:
            if foe.name == nameOfFoe and howMany < 2:
                return foe
            elif foe.name == nameOfFoe:
                howmany -= 1
                extraFoes.add(foe)
        #---------------------------
        if len(extraFoes) > 0:
    ...
    def checkIfBelong( dude2Check , spcecialGroup , doesItMatter ):#This method may be removed due to not being needed
    ...
    def removeDedFoes( enemyTroopers , ringLeaders , alternativeTargets , ringLeaders2Kill , alternativeTargets2Kill , endOnRingLeadersGone , winWhenAlternativeTargetsKilled ):
        for foe in enemyTroopers:
            
            if foe.health <= 0:
                if foe in ringLeaders and endOnRingLeadersGone:
                    ringLeaders.remove(foe)
                    

                elif foe in alternativeTargets and winWhenAlternativeTargetsKilled:
                    
                    alternativeTargets.remove(foe)

                if isinstance( foe , ChariotFoe ):                
                    foe.spawnTransportTroopers( foe.transportFoes , enemyTroopers )
                
                enemyTroopers.remove(foe)
#----------------------------------------------------------------------------------------------------------------------------------------
            ...
            frame:
                ...
                vbox:
                    ...
                    hbox:
                        ...
                        if partyMember.mount.mountArmor != "":
                        ...
                        if check4Effect( "Charged" , partyMember.effects ):
                            image Transform( "/gui/Charged.png" , zoom = 0.33 )
                        if check4Effect( "OverCharged" , partyMember.effects ):
                            image Transform( "/gui/OverCharged.png" , zoom = 0.33 )
                        if check4Effect( "Clears" , partyMember.effects ):
                            image Transform( "/gui/Clearing.png" , zoom = 0.33 )
            ...
            if len(enemyTroopers) > 0:
                ...
                if currentParty[i].health > 0 and canPlayTurn:
                    ...
                    menu:
                        ...
                        #-------------------------------------------------------------------------------
                        "Overcharge the Sword of Ahura-Mazda (Costs 40 Helath)" if currentPlayer.weapon.type == "SoAM" and not check4Effect( "OverCharged" , currentPlayer.effects ) and xerxCanOverCharge and currentPlayer.health > 50:
                            $ addEffects( "Charged" , currentPlayer , 1 , 10 , "Sword of Ahura-Mazda" )
                        ...
                        "Unlesh the Power!" if currentPlayer.weapon.type == "SoAM" and check4Effect( "OverCharged" , currentPlayer.effects ):
                            menu:
                                "Chop (Single Target)":
                                    "Who will feel the Power of Ahura-Mazda?"
                                    hide screen statusScreen
                                    call screen choose2Attack( "Chop Overcharge" )
                                    show screen statusScreen
                                    $ theTarget = _return
                                    $ foe2Die = enemyTroopers[theTarget]
                                    $ damageNumber = foe2Die.health
                                    $ rythmAttacking ( enemyTroopers[theTarget] , currentPlayer , 5.0 )
                                    $ damageNumber = damageNumber - foe2Die.health 
                                    queue sound flameSlice
                                    play extraSound foeHit
                                    "[foe2Die.name] took [damageNumber] damage"
                                    if foe2Die.health <= 0:
                                        play sound meatEplosion
                                        play extraSound magicAttackUnchmabered
                                        "[foe2Die.name] was OBLITERATED!!"
                                        $ removeDedFoes( enemyTroopers , ringLeaders , alternativeTargets , ringLeaders2Kill , alternativeTargets2Kill , endOnRingLeadersGone , winWhenAlternativeTargetsKilled )
                                    

                                "Slice (Area of Effect/Multiple enemies)":
                                    "Aim the Center of the Slice"
                                    hide screen statusScreen
                                    call screen choose2Attack( "Slice Overcharge" )
                                    show screen statusScreen
                                    ...
                                    while splash < len(enemyTroopers):
                                        ...
                                        $ splashFactor = 2.0 - (0.5 * splashDistance )

                                        #damage should be more then the bombs (or maybe the same?)
                                        # 2.0 - 1.5 - 1.0 - 0.5 - -0.0=0.0
                                        #2*2*3 = 2 4 12 - 12 * 10 = 120 dmg 10 ap
                                        #1.5*2 = 3 3*3 = 9*10 = 90 dmg 10 ap
                                        #1*2*3 = 6*10 = 60 dmg 10 ap
                                        #0.5*2*3 = 3*10 = 30 dmg 10 ap
                                        #120 + 90 + 60 + 30 = 120 + 180 = 300
                                        #300/10 = 30
                                        ...
                                        if splashFactor < 0:
                                            $ splashFactor = 0
                                        if splashFactor > 0:
                                            $ rythmAttacking ( enemyTroopers[splash] , currentPlayer , splashFactor * 1.5 )
                                        ...
                                        if originalHealth > currentDude.health:
                                            if currentDude.health > 0:
                                                ...
                                                play sound flameSlice
                                                play extraSound foeHit
                                                "[currentDude.name] got sliced for [blasted4] damage"
                                            
                                            elif currentDude.health <= 0:
                                                play sound meatEplosion
                                                play extraSound magicAttackUnchmabered

                                                "[currentDude.name] SLICED IN HALF!!"
                                                $ removeDedFoes( enemyTroopers , ringLeaders , alternativeTargets , ringLeaders2Kill , alternativeTargets2Kill , endOnRingLeadersGone , winWhenAlternativeTargetsKilled )
                                                $ splash -= 1
                                                $ splashCenter -= 1
                                        $ splash += 1
                            $ removeEffectType( "OverCharged" , currentPlayer.effects )
                        #-------------------------------------------------------------------------------
